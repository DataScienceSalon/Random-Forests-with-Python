...
```{python libraries, code = readLines("../ida.py")[7:15]}
```
...
```{python ida_select, code = readLines("../ida.py")[38:45]}
```
...
### Compliance  
```{python compliance, code = readLines("../ida.py")[50:60]}
```
...
```{python compliance_plot, code = readLines("../ida.py")[62:63]}
```
...
### Agency   
```{python agency, code = readLines("../ida.py")[69:75]}
```
...
```{python agency_plot, code = readLines("../ida.py")[77:78]}
```
...
### Inspector
```{python inspector, code = readLines("../ida.py")[85:91]}
```
...
```{python inspector_plot, code = readLines("../ida.py")[93:94]}
```
...
Since the violator name is manually entered on each ticket by the inspector, it is subject to omission as is indicated by the `r py$violator_missing` missing values, and/or spelling variation which could distort any analysis based upon this variable. Insofar as we would like to use variable name as a basis for deriving new variables, such as the number of past violations,  we'll need to make some assumptions. First, we'll assume any missing violator names can be imputed from the street number and address of the violation. Second, we will assume any variations in spelling are intentional.

* * *  

### Address Variables
As shown in `r kfigr::figr(label = "vars", prefix = TRUE, link = TRUE, type="Figure")`, there are no null values for the violation street number and name variables. Mailing address street number and name are subject to omission with 
...
4. Impute missing violator names with the street number and address of the violation.
