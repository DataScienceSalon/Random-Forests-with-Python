...
For both the missing and presumably incorrect hearing dates, new hearing dates may be imputed based upon the mean interval between ticket issued dates and hearing dates, overall or at the violation code level.
...
With respect to data preparation, the following treatments were identified: 
...
3. Missing states for US based violators should be imputed as 'MI' for Michigan.  
4. Impute missing or incorrect hearing dates using the mean interval date for the violation code.
5. Reduce the categorical levels of zip code with a new region variable which contains the first three numbers of the zip code.
4. Conduct thorough assessments of outliers for any variables considered for modeling.  

New features should be created to capture total tickets written and compliance rates for the following categorical variables:  
* Agency
* Inspector
* Violation Code
* Violator
...
Once we've measured tickets and compliance rates, we can introduce new variables which characterize agency, inspector, violator, violation code and street compliance.
